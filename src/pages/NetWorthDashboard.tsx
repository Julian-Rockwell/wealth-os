        ...
        <div>
          ...
          <Tabs defaultValue="holdings" className="w-full">
              ...
              <TabsContent value="liabilities" className="mt-4">
                ...
                <LiabilitiesTable liabilities={snapshot.liabilities} onUpdate={(id, updates) => setSnapshot(updateSnapshotLiabilities(snapshot, snapshot.liabilities.map(l => l.id === id ? { ...l, ...updates } : l)))} onDelete={(id) => setSnapshot(updateSnapshotLiabilities(snapshot, snapshot.liabilities.filter(l => l.id !== id)))} onAdd={() => setSnapshot(updateSnapshotLiabilities(snapshot, [...snapshot.liabilities, { id: `liability-${Date.now()}`, accountId: "manual-1", name: "New Liability", type: "other", apr: 0, balance: 0, monthlyPayment: 0, remainingTermMonths: 0 }]))} />
              </TabsContent>
            </Tabs></div>
          </Card>
        </div>
